...
def run_semantic_pipeline(
    ...
    """
    SDIE v3.3 — Structural Component Intelligence pipeline.
    DXF → classify components → graph → semantic model → slab intelligence → quantities.
    """
    ...
    atlas = load_atlas(config.atlas_path)
    ...
    result = {
        "engine": "SDIE",
        "version": "3.3.0",
        "architecture": "structural_component_intelligence",
    ...
    overlay_files = write_overlay_outputs(
        ...
        slabs,
        meta.extents,
        title=f"SDIE v3.3 — {dxf_path.name}",
        totals=totals,
        excluded_wkt=exclusion_union_wkt,
    )

    summary_lines = [
        f"SDIE v3.3 Semantic Pipeline — {dxf_path.name}",
