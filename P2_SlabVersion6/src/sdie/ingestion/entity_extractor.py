...
def _hatch_bounds(entity) -> tuple[float, float, float, float] | None:
    """Bounding box from hatch paths (fallback when entity.bbox() fails)."""
    xs: list[float] = []
    ys: list[float] = []
    try:
            ...
            if hasattr(path, "vertices"):
                ...
                for edge in path.edges:
                    if edge.EDGE_TYPE == "LineEdge":
                        xs.extend([edge.start[0], edge.end[0]])
                        ys.extend([edge.start[1], edge.end[1]])
    except Exception:
        return None
    if not xs or not ys:
        return None
    ...
    if entity.dxftype() == "HATCH":
        ...
        if not b:
            try:
                ext = entity.bbox()
                if ext:
                    b = (ext.extmin.x, ext.extmin.y, ext.extmax.x, ext.extmax.y)
            except Exception:
                b = None
        if b:
            return b
