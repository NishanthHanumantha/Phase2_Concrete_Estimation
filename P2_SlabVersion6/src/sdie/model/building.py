...
def build_semantic_model(
    *,
    ...
    beam_quantities: list[dict] | None = None,
    ...
    if beam_quantities:
        qty["beams"] = beam_quantities
        qty["beam_totals"] = {
            "beam_count": len(beam_quantities),
            "total_length_m": round(
                sum(b.get("length_m", 0) for b in beam_quantities), 3
            ),
            "concrete_m3": round(
                sum(b.get("concrete_m3", 0) for b in beam_quantities), 6
            ),
            "shuttering_m2": round(
                sum(b.get("shuttering_m2", 0) for b in beam_quantities), 6
            ),
        }
    if slab_quantities or beam_quantities:
        qty["totals"] = {
            "slab_count": len(slab_quantities or []),
            "beam_count": len(beam_quantities or []),
            "area_m2": round(sum(s.get("area_m2", 0) for s in (slab_quantities or [])), 6),
            "concrete_m3": round(
                sum(s.get("concrete_m3", 0) for s in (slab_quantities or []))
                + sum(b.get("concrete_m3", 0) for b in (beam_quantities or [])),
                6,
            ),
            "shuttering_m2": round(
                sum(s.get("shuttering_m2", 0) for s in (slab_quantities or []))
                + sum(b.get("shuttering_m2", 0) for b in (beam_quantities or [])),
                6,
            ),
        }
