...
from sdie.ingestion.layer_discovery import apply_layer_plan_to_config
...
def run_pipeline(
    ...
    if config.use_semantic_pipeline:
        from sdie.semantic_pipeline import run_semantic_pipeline

        return run_semantic_pipeline(dxf_path, output_dir, config)

    output_dir.mkdir(parents=True, exist_ok=True)

    doc, meta = load_drawing(dxf_path)
    msp = doc.modelspace()
    apply_layer_plan_to_config(config, msp)
