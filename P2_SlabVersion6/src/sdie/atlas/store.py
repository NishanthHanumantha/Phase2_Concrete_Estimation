...
def load_atlas(
    ...
    *,
    ...
    out = [AtlasSample.from_dict(s) for s in samples]
    if project_id and not is_generic_project(project_id):
        out = [s for s in out if s.project_id in (project_id, "GLOBAL")]
    return out
