...
from sdie.util.progress import current_progress
...
# Void/supporting types resolved by layer name or annotation — keep out of DeepSeek beam/slab drift.
SUPPORTING_VOID_TYPES: frozenset[ComponentType] = frozenset(
    {
...
def _is_locked_supporting_type(comp: ClassifiedComponent) -> bool:
    if comp.component_type not in SUPPORTING_VOID_TYPES:
        return False
    return any(
        e.startswith(("hard_layer:", "void_keyword:")) for e in comp.evidence
    )
...
def _is_ambiguous(
    ...
    *,
    ...
    if _is_locked_supporting_type(comp):
        return False
...
def classify_entities_v4(
    ...
    *,
    ...
    total_batches = max(1, (len(rag_payload) + batch_size - 1) // batch_size)
    for batch_idx, i in enumerate(range(0, len(rag_payload), batch_size), start=1):
        batch = rag_payload[i : i + batch_size]
        current_progress().substep(
            batch_idx,
            total_batches,
            "DeepSeek (ambiguous only)",
            base_pct=35.0,
            span_pct=20.0,
            detail=f"batch {batch_idx}/{total_batches}, {len(batch)} entities",
        )
        user_msg = (
...
def _format_alt_label(alt: Any) -> str:
    if isinstance(alt, str):
        return alt
    if isinstance(alt, dict):
        name = alt.get("component") or alt.get("type") or alt.get("label") or "?"
...
def _apply_v5_deepseek_results(
    ...
    for item in items:
        ...
        if alts:
            labels = [_format_alt_label(a) for a in alts[:3]]
            comp.evidence = comp.evidence + [f"alternatives:{','.join(labels)}"]
...
def classify_entities_v5(
    ...
    *,
    ...
    rag_payload = []
    amb_sorted = sorted(ambiguous_ids)
    for idx, cid in enumerate(amb_sorted, start=1):
        entity = entity_by_id.get(cid)
        if entity is None:
            continue
        if idx == 1 or idx == len(amb_sorted) or idx % 200 == 0:
            current_progress().substep(
                idx,
                len(amb_sorted),
                "RAG context for ambiguous",
                base_pct=33.0,
                span_pct=2.0,
            )
