...
def _geometry_classify(
    ...
    if wall_s >= 0.6:
        layer_u = layer.upper()
        if any(k in layer_u for k in ("SHEAR", "SW", "WALL", "S-BEAM", "STR-BEAM")):
            return ComponentType.SHEAR_WALL, wall_s, ["geometry:wall_line"]
        return ComponentType.STRUCTURAL_WALL, wall_s, ["geometry:wall_line"]
    if entity.entity_type in ("LWPOLYLINE", "POLYLINE") and entity.length_mm:
        if entity.length_mm >= 2500 and (entity.aspect_ratio or 1.0) <= 6.0:
            return ComponentType.SHEAR_WALL, 0.78, ["geometry:wall_polyline"]
    return None, 0.0, []
...
def classify_entity(
    ...
    *,
    ...
    if hard and hard[0] in (
        ...
        evidence.append(tag)
    elif ann_feats.get("void_component_hint"):
        ...
        soft_layers = set(profiles.get("soft_layers") or SOFT_LAYER_HINTS)
        soft_layer = layer in soft_layers

        if soft_layer:
            gtype, gconf, gev = _geometry_classify(entity, layer)
            proj_rule = resolve_layer_rule(
                project_id, layer, etype, profiles=profiles
            )
            if gtype == ComponentType.BEAM and gconf >= 0.88:
                component_type, rule_conf = gtype, gconf
                evidence.extend(gev)
                matched = True
            elif (
                proj_rule
                and proj_rule[0] == ComponentType.SHEAR_WALL
                and proj_rule[1] >= 0.85
            ):
                component_type, rule_conf, tag = proj_rule
                evidence.append(tag)
                matched = True
            elif gtype and gconf >= 0.82:
                component_type, rule_conf = gtype, gconf
                evidence.extend(gev)
                matched = True
            elif proj_rule:
        ...
        if not matched and layer in hints and layer not in soft_layers:
    ...
    locked_void = component_type in (
        ComponentType.OPENING,
        ComponentType.LIFT_CORE,
        ComponentType.STAIR_CORE,
        ComponentType.SHAFT,
    ) and any(e.startswith(("hard_layer:", "void_keyword:")) for e in evidence)
