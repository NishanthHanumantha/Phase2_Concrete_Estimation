...
from sdie.classification.layer_profiles import (
    ...
    save_profiles,
    save_profiles_merging_projects,
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT,
        help="P2_SlabVersion6 root (Revised Project Knowledge paths resolve here)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="ID",
        help="Only learn rules for these manifest project_ids (repeatable)",
    )
    parser.add_argument(
        "--merge-projects",
        nargs="+",
        default=[],
        metavar="ID",
        help="Replace rules for these project_ids only; keep others from existing output",
    )
    ...
    if args.merge_projects:
        out = save_profiles_merging_projects(
            rules,
            args.output,
            merge_project_ids=set(args.merge_projects),
        )
    else:
    ...
    summary = {
        ...
        "merged_projects": args.merge_projects or None,
    }
